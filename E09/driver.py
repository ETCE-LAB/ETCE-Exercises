...
def check_tx(t):
    ...
    try:
        ...
        print("Could not get string representation, check Ex08Transaction.__str__")
        traceback.print_exc()
        grades[0] = False
    ...
    try:
        ...
        print("Could not get transaction hash, check Ex08Transaction.hash")
        traceback.print_exc()
        grades[1] = False
    ...
    try:
        if not (str_rep1 != str_rep2 != str_rep3):
            print("String representation is not unique, check Ex08Transaction.__str__")
            grades[0] = False
    except Exception as E:
        traceback.print_exc()
    ...
    try:
        if not (isinstance(hash1, _hashlib.HASH) and
            ...
            print("Hashes are not valid _hashlib.HASH instances, check Ex08Transaction.hash")
            grades[1] = False
    except Exception as E:
        traceback.print_exc()
    ...
    try:
        if not (hash1.digest() != hash2.digest() != hash3.digest()):
            print("Hashes are not true hashes, check Ex08Transaction.hash")
            grades[2] = False
    except Exception as E:
        traceback.print_exc()
...
def check_block(t, b):
    ...
    try:
        ...
        print("Could not get block string representation, check Ex08Block.__str__")
        traceback.print_exc()
        grades[3] = False
    ...
    try:
        ...
        print("Could not get block hash, check Ex08Block.hash")
        traceback.print_exc()
        grades[4] = False

    try:
        if not (str_rep1 != str_rep2):
            print("String representation of Block is not unique, check Ex08Block.__str__")
            grades[3] = False
    except Exception as E:
        print("Could not get block string representation, check Ex08Block.__str__")
        traceback.print_exc()
    ...
    try:
        if not (isinstance(hash1, _hashlib.HASH) and
            ...
            print("Block hash is not a valid _hashlib.HASH instance, check Ex08Block.hash")
            grades[4] = False
    ...
    try:
        if not (hash1 != hash2):
            print("Block hash is not a true hash, check Ex08Block.hash")
            grades[4] = False
    except Exception as E:
        print("Could not get block hash, check Ex08Block.hash")
        traceback.print_exc()
